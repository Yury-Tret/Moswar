...
//                Bot.GrpFight.Val = (chkGoPVPFight.Checked ? 1 : 0) + (chkGoNPCFight.Checked ? 2 : 0); // 0-> Выкл., 1-> PVP, 2-> NPC, 3-> Все.
        ...
        private void frmBrowser_Load(object sender, EventArgs e)
        {
            ...
            Tip.SetToolTip(btnShowRunPet, "К настройкам беговых питомцев.");
            Tip.SetToolTip(btnShowWarPet, "К настройкам боевых питомцев.");
            Tip.SetToolTip(btnShowShutdown, "К настройкам выключения компьютера.");
            Tip.SetToolTip(btnShowShaurburgers, "К настройкам шаурбургерса.");
            Tip.SetToolTip(btnShowWerewolf, "К настройкам оборотеня в погонах.");
            Tip.SetToolTip(btnShowAlley, "К настройкам Аллеи.");
            Tip.SetToolTip(btnShowSearchRat, "К настройкам охоты на крысомах.");
            Tip.SetToolTip(btnShowDigRat, "К настройкам копания в метро.");
            Tip.SetToolTip(btnShowThimbleSettings, "К дополнительным настройкам средств перевода денег в руду.");
            Tip.SetToolTip(btnShowThimblePlay, "К настройкам игры с Моней Шацом.");
            Tip.SetToolTip(btnShowGrpFightSettings, "К дополнительным настройкам груповых боёв.");
            Tip.SetToolTip(btnShowGrpFightSettings, "К настройкам груповых боёв.");
            Tip.SetToolTip(btnShowQuestSettings, "К дополнительным настройкам квэстов.");
            Tip.SetToolTip(btnShowQuest, "К настройкам квэстов.");
            Tip.SetToolTip(numStatDiff, "Максимальная разница характеристик,\nбез учёта харизмы.");
            Tip.SetToolTip(btnShowCasinoSettings, "К дополнительным настройкам казино.");
            Tip.SetToolTip(btnShowCasino, "К настройкам казино.");            
...
//            if (LastUpdateCheckDT < DateTime.Now) CheckForUpdate();
            ...
            {                
                Tip.SetToolTip(statusStrip,
                    ...
                    + (Bot.Settings.Quest ? "Квэст:\nВыполнение заданий приостановлено: " + (Bot.Me.Events.StopQuest || !Bot.Settings.Quest) + "\n\n" : "")
       ...
       private void OilType_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (sender.Equals(chkGoOil))
            {
                ...
                btnShowOilSettings.Enabled = true;
            }
            #endregion
            #region Ленинопровод
            ...
            #endregion
            btnShowOilSettings.Enabled = ((CheckBox)sender).Checked;
        }
        ...
        {
            #region Беговой питомец
            if (sender.Equals(btnShowRunPet))
            {
                btnShowRunPet.Enabled = false;                
                grpRunPet.Visible = true;
                do
                {
                    grpRunPet.Left += 1;
                    Application.DoEvents();
                }
                while (grpRunPet.Left < GrpWarPet.Left);
                btnShowWarPet.Enabled = true;
            }
            #endregion 
            #region Боевой питомец
            if (sender.Equals(btnShowWarPet))
            {
                btnShowWarPet.Enabled = false;
                do
                {
                    grpRunPet.Left -= 2;
                    Application.DoEvents();
                }
                while (grpRunPet.Left > -200);
                grpRunPet.Visible = false;
                btnShowRunPet.Enabled = true;
                grpRunPet.Left = -160;
            }
            #endregion
            #region ShutDown
            if (sender.Equals(btnShowShutdown))
            {
                btnShowShutdown.Enabled = false;
                ...
                while (grpShutDown.Left < grpShaurburgers.Left);
                btnShowShaurburgers.Enabled = true;
            }
            #endregion
            #region MC
            if (sender.Equals(btnShowShaurburgers))
            {
                btnShowShaurburgers.Enabled = false;
                do
                {
                    grpShutDown.Left -= 2;
                    Application.DoEvents();
                }
                while (grpShutDown.Left > -300);
                grpShutDown.Visible = false;
                btnShowShutdown.Enabled = true;
                grpShutDown.Left = -260;
            }
            #endregion
            #region Дополнительные настройки Proxy
            if (sender.Equals(btnShowProxySettings))
            {
                btnShowProxySettings.Enabled = false;
                grpProxySettings.Visible = true;
                do
                {
                    grpProxySettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpProxySettings.Left < grpProxy.Left);
                btnShowProxy.Enabled = true;
            }
            #endregion
            #region Обычные настройки Proxy
            if (sender.Equals(btnShowProxy))
            {
                btnShowProxy.Enabled = false;
                do
                {
                    grpProxySettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpProxySettings.Left > -260);
                grpProxySettings.Visible = false;
                btnShowProxySettings.Enabled = true;
                grpProxySettings.Left = -220;
            }
            #endregion
            #region Дополнительные настройки петриков
            if (sender.Equals(btnShowPetrikiSettings))
            {
                btnShowPetrikiSettings.Enabled = false;
                grpPetrikiSettings.Visible = true;
                do
                {
                    grpPetrikiSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpPetrikiSettings.Left < grpPetriki.Left);
                btnShowPetriki.Enabled = true;
            }
            #endregion
            #region Обычные настройки петриков
            if (sender.Equals(btnShowPetriki))
            {
                btnShowPetriki.Enabled = false;
                do
                {
                    grpPetrikiSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpPetrikiSettings.Left > -300);
                grpPetrikiSettings.Visible = false;
                btnShowPetrikiSettings.Enabled = true;
                grpPetrikiSettings.Left = -260;
            }
            #endregion
            #region Охота на крысомах
            if (sender.Equals(btnShowSearchRat))
            {
                btnShowSearchRat.Enabled = false;
                grpSearchRat.Visible = true;
                do
                {
                    grpSearchRat.Left += 1;
                    Application.DoEvents();
                }
                while (grpSearchRat.Left < grpDig.Left);
                btnShowDigRat.Enabled = true;
            }
            #endregion
            #region Копание + Крысы
            if (sender.Equals(btnShowDigRat))
            {
                btnShowDigRat.Enabled = false;
                do
                {
                    grpSearchRat.Left -= 2;
                    Application.DoEvents();
                }
                while (grpSearchRat.Left > -400);
                grpSearchRat.Visible = false;
                btnShowSearchRat.Enabled = true;
                grpSearchRat.Left = -360;
            }
            #endregion
            #region Дополнительные настройки мони и слива денег
            if (sender.Equals(btnShowThimbleSettings))
            {
                btnShowThimbleSettings.Enabled = false;
                grpThimbleSettings.Visible = true;
                do
                {
                    grpThimbleSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpThimbleSettings.Left < grpThimblePlay.Left);
                btnShowThimblePlay.Enabled = true;
            }
            #endregion
            #region Игра с моней
            if (sender.Equals(btnShowThimblePlay))
            {
                btnShowThimblePlay.Enabled = false;
                do
                {
                    grpThimbleSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpThimbleSettings.Left > -300);
                grpThimbleSettings.Visible = false;
                btnShowThimbleSettings.Enabled = true;
                grpThimbleSettings.Left = -260;
            }
            #endregion
            #region Дополнительные настройки стенок
            if (sender.Equals(btnShowGrpFightSettings))
            {
                btnShowGrpFightSettings.Enabled = false;
                grpGrpFightSettings.Visible = true;
                do
                {
                    grpGrpFightSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpGrpFightSettings.Left < grpGrpFight.Left);
                btnShowGrpFight.Enabled = true;
            }
            #endregion
            #region Обычные настройки стенок
            if (sender.Equals(btnShowGrpFight))
            {
                btnShowGrpFight.Enabled = false;
                do
                {
                    grpGrpFightSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpGrpFightSettings.Left > -300);
                grpGrpFightSettings.Visible = false;
                btnShowGrpFightSettings.Enabled = true;
                grpGrpFightSettings.Left = -260;
            }
            #endregion
            #region Дополнительные настройки закоулки
            if (sender.Equals(btnShowWerewolf))
            {
                btnShowWerewolf.Enabled = false;
                grpWerewolf.Visible = true;
                do
                {
                    grpWerewolf.Left += 1;
                    Application.DoEvents();
                }
                while (grpWerewolf.Left < grpAlley.Left);
                btnShowAlley.Enabled = true;
            }
            #endregion
            #region Обычные настройки закоулки
            if (sender.Equals(btnShowAlley))
            {
                btnShowAlley.Enabled = false;
                do
                {
                    grpWerewolf.Left -= 2;
                    Application.DoEvents();
                }
                while (grpWerewolf.Left > -300);
                grpWerewolf.Visible = false;
                btnShowWerewolf.Enabled = true;
                grpWerewolf.Left = -260;
            }
            #endregion
            #region Дополнительные настройки Квэст
            if (sender.Equals(btnShowQuestSettings))
            {
                btnShowQuestSettings.Enabled = false;
                grpQuestSettings.Visible = true;
                do
                {
                    grpQuestSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpQuestSettings.Left < grpQuest.Left);
                btnShowQuest.Enabled = true;
            }
            #endregion
            #region Обычные настройки Квэст
            if (sender.Equals(btnShowQuest))
            {
                btnShowQuest.Enabled = false;
                do
                {
                    grpQuestSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpQuestSettings.Left > -240);
                grpQuestSettings.Visible = false;
                btnShowQuestSettings.Enabled = true;
                grpQuestSettings.Left = -200;
            }
            #endregion
            #region Дополнительные настройки Казино
            if (sender.Equals(btnShowCasinoSettings))
            {
                btnShowCasinoSettings.Enabled = false;
                grpCasinoSettings.Visible = true;
                do
                {
                    grpCasinoSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpCasinoSettings.Left < grpCasino.Left);
                btnShowCasino.Enabled = true;
            }
            #endregion
            #region Обычные настройки Казино
            if (sender.Equals(btnShowCasino))
            {
                btnShowCasino.Enabled = false;
                do
                {
                    grpCasinoSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpCasinoSettings.Left > -240);
                grpCasinoSettings.Visible = false;
                btnShowCasinoSettings.Enabled = true;
                grpCasinoSettings.Left = -200;
            }
            #endregion
            #region Дополнительные настройки Допингов
            if (sender.Equals(btnShowDopingSettings))
            {
                btnShowDopingSettings.Enabled = false;
                grpDopingSettings.Visible = true;
                do
                {
                    grpDopingSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpDopingSettings.Left < grpDoping.Left);
                btnShowDoping.Enabled = true;
            }
            #endregion
            #region Обычные настройки Допингов
            if (sender.Equals(btnShowDoping))
            {
                btnShowDoping.Enabled = false;
                do
                {
                    grpDopingSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpDopingSettings.Left > -240);
                grpDopingSettings.Visible = false;
                btnShowDopingSettings.Enabled = true;
                grpDopingSettings.Left = -200;
            }
            #endregion
            #region Дополнительные настройки HC
            if (sender.Equals(btnShowHCSettings))
            {
                btnShowHCSettings.Enabled = false;
                grpHCSettings.Visible = true;
                do
                {
                    grpHCSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpHCSettings.Left < grpHC.Left);
                btnShowHC.Enabled = true;
            }
            #endregion
            #region Обычные настройки HC
            if (sender.Equals(btnShowHC))
            {
                btnShowHC.Enabled = false;
                do
                {
                    grpHCSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpHCSettings.Left > -300);
                grpHCSettings.Visible = false;
                btnShowHCSettings.Enabled = true;
                grpHCSettings.Left = -260;
            }
            #endregion
            #region Нефтепровод / Ленинопровод
            if (sender.Equals(btnShowOilSettings))
            {
                btnShowOilSettings.Enabled = false;
                #region Нефтепровод
                if (chkGoOil.Checked)
                {
                    grpOilSettings.Visible = true;
                    do
                    {
                        grpOilSettings.Left += 1;
                        Application.DoEvents();
                    }
                    while (grpOilSettings.Left < grpOil.Left);
                    btnShowOil.Enabled = true;
                }
                #endregion
                #region Ленинопровод
                if (chkGoOilLenin.Checked)
                {
                    grpOilLeninSettings.Visible = true;
                    do
                    {
                        grpOilLeninSettings.Left += 1;
                        Application.DoEvents();
                    }
                    while (grpOilLeninSettings.Left < grpOil.Left);
                    btnShowOilLenin.Enabled = true;
                }
                #endregion
            }
            #endregion
            #region Обычные настройки нефтепровода
            if (sender.Equals(btnShowOil))
            {
                btnShowOil.Enabled = false;
                do
                {
                    grpOilSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpOilSettings.Left > -300);
                grpOilSettings.Visible = false;
                btnShowOilSettings.Enabled = true;
                grpOilSettings.Left = -260;                
            }
            if (sender.Equals(btnShowOilLenin))
            {
                btnShowOil.Enabled = false;
                do
                {
                    grpOilLeninSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpOilLeninSettings.Left > -300);
                grpOilLeninSettings.Visible = false;
                btnShowOilSettings.Enabled = true;
                grpOilLeninSettings.Left = -260;
            }            
            #endregion
            #region Дополнительные настройки Clanwar
            if (sender.Equals(btnShowClanWarExSettings))
            {
                btnShowClanWarExSettings.Enabled = false;
                grpClanWarExSettings.Visible = true;
                do
                {
                    grpClanWarExSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpClanWarExSettings.Left < grpClanWarEx.Left);
                btnShowClanWarEx.Enabled = true;
            }
            #endregion
            #region Обычные настройки Clanwar
            if (sender.Equals(btnShowClanWarEx))
            {
                btnShowClanWarEx.Enabled = false;
                do
                {
                    grpClanWarExSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpClanWarExSettings.Left > -410);
                grpClanWarExSettings.Visible = false;
                btnShowClanWarExSettings.Enabled = true;
                grpClanWarExSettings.Left = -370;
            }
            #endregion
            #region Дополнительные настройки Azazella
            if (sender.Equals(btnShowAzazellaSettings))
            {
                btnShowAzazellaSettings.Enabled = false;
                grpAzazellaSettings.Visible = true;
                do
                {
                    grpAzazellaSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpAzazellaSettings.Left < grpAzazella.Left);
                btnShowAzazella.Enabled = true;
            }
            #endregion
            #region Обычные настройки Azazella
            if (sender.Equals(btnShowAzazella))
            {
                btnShowAzazella.Enabled = false;
                do
                {
                    grpAzazellaSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpAzazellaSettings.Left > -230);
                grpAzazellaSettings.Visible = false;
                btnShowAzazellaSettings.Enabled = true;
                grpAzazellaSettings.Left = -190;
            }
            #endregion
            #region Дополнительные настройки AutoCoctail
            if (sender.Equals(btnShowAutoCoctailSettings))
            {
                btnShowAutoCoctailSettings.Enabled = false;
                grpAutoCoctailSettings.Visible = true;
                do
                {
                    grpAutoCoctailSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpAutoCoctailSettings.Left < grpAutoCoctail.Left);
                btnShowAutoCoctail.Enabled = true;
            }
            #endregion
            #region Обычные настройки AutoCoctail
            if (sender.Equals(btnShowAutoCoctail))
            {
                btnShowAutoCoctail.Enabled = false;
                do
                {
                    grpAutoCoctailSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpAutoCoctailSettings.Left > -260);
                grpAutoCoctailSettings.Visible = false;
                btnShowAutoCoctailSettings.Enabled = true;
                grpAutoCoctailSettings.Left = -220;
            }
            #endregion
            #region Обычные настройки Имитации человека
            if (sender.Equals(btnShowHumanEmulationEx))
            {
                btnShowHumanEmulationEx.Enabled = false;
                grpHumanEmulationEx.Visible = true;
                do
                {
                    grpHumanEmulationEx.Left += 1;
                    Application.DoEvents();
                }
                while (grpHumanEmulationEx.Left < grpHumanEmulation.Left);
                btnShowHumanEmulation.Enabled = true;
            }
            #endregion
            #region Дополнительные настройки Имитации человека
            if (sender.Equals(btnShowHumanEmulation))
            {
                btnShowHumanEmulation.Enabled = false;
                do
                {
                    grpHumanEmulationEx.Left -= 2;
                    Application.DoEvents();
                }
                while (grpHumanEmulationEx.Left > -180);
                grpHumanEmulationEx.Visible = false;
                btnShowHumanEmulationEx.Enabled = true;
                grpHumanEmulationEx.Left = -160;
            }
            #endregion
            #region Обычные настройки противостояния
            if (sender.Equals(btnShowProtivostoyanieSettings))
            {
                btnShowProtivostoyanieSettings.Enabled = false;
                grpProtivostoyanieSettings.Visible = true;
                do
                {
                    grpProtivostoyanieSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpProtivostoyanieSettings.Left < grpProtivostoyanie.Left);
                btnShowProtivostoyanie.Enabled = true;
            }
            #endregion
            #region Дополнительные настройки противостояния
            if (sender.Equals(btnShowProtivostoyanie))
            {
                btnShowProtivostoyanie.Enabled = false;
                do
                {
                    grpProtivostoyanieSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpProtivostoyanieSettings.Left > -180);
                grpProtivostoyanieSettings.Visible = false;
                btnShowProtivostoyanieSettings.Enabled = true;
                grpProtivostoyanieSettings.Left = -160;
            }
            #endregion
            #region Обычные настройки заполнения боевого инвентаря
            if (sender.Equals(btnShowExpGrpFightSettings))
            {
                btnShowExpGrpFightSettings.Enabled = false;
                grpExpGrpFightSettings.Visible = true;
                do
                {
                    grpExpGrpFightSettings.Left += 1;
                    Application.DoEvents();
                }
                while (grpExpGrpFightSettings.Left < grpExpGrpFight.Left);
                btnShowExpGrpFight.Enabled = true;
            }
            #endregion
            #region Дополнительные настройки заполнения боевого инвентаря
            if (sender.Equals(btnShowExpGrpFight))
            {
                btnShowExpGrpFight.Enabled = false;
                do
                {
                    grpExpGrpFightSettings.Left -= 2;
                    Application.DoEvents();
                }
                while (grpExpGrpFightSettings.Left > -240);
                grpExpGrpFightSettings.Visible = false;
                btnShowExpGrpFightSettings.Enabled = true;
                grpExpGrpFightSettings.Left = -200;
            }
            #endregion
        }       
